...
let inputDay;
let dayList = document.querySelectorAll('.singleDay');
let button  = document.querySelector('button');

button.addEventListener('click', ()=>{
    //console.log("func called")
    inputDay = document.querySelector('input').value;
    console.log(dayList.length);
    for(let i=0;i<dayList.length;i++){
        if(dayList[i].innerHTML==inputDay){
            console.log(dayList[i]);
            dayList[i].style.background="green";
        }
        //console.log(dayList[i].innerHTML);
